...
Shortcode coalescing is a conditional expression of the form:
...
```json
{
  ...
  "full_name": "{{user.first_name || 'John'}} {{user.last_name || 'Doe' }}",
...
The output after substitution (subject to the actual user data) would look like this:
...
Coalescing is often used to substitute a default fallback value for shortcodes that would otherwise result in `null` or `false` values.

In a coalescing expression, operands are separated by `||`. The result of a coalescing expression is the first operand that is not `null`/`false`. If all operands of the expression are `null`/`false`, the last operand is returned no matter its return value.

Coalescing can be combined with interpolation:

```json
{
  "title": "Awesome {{user.first_name || user.last_name || 'user'}}"
}
```
...
The rules that sum up how coalescing works are:

- string operands should use single quotes (`'`)
- the result of a coalescing expression is either the first operand that is not `null`/`false`, or the last operand (no matter its value)
- strings, booleans and numbers are allowed as fallback (i.e. non-shortcode) operands
